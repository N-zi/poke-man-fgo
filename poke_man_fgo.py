...
from hoshino.modules.poke_man_fgo import chara, _data
...
DIR_PATH = os.path.join(os.path.expanduser(
    hoshino.config.RES_DIR), 'img', 'fgo_unit')
DB_PATH = os.path.expanduser("~/.hoshino/poke_man_fgo.db")
...
BLACKLIST_CARD = ['icon_unit_100031.png','icon_unit_171031.png']
...
sv = Service('poke-man-fgo', bundle='pcr娱乐', help_='''
戳一戳机器人, 她可能会送你fgo卡片哦~
...
for image in image_list:
    ...
    if chara_id == 1000 or chara_id not in _data.CHARA_NAME:
        continue
    star = image[14]
...
async def poke_back(session: NoticeSession):
    ...
    if not daily_limiter.check(guid) or random.random() > POKE_GET_CARDS:
        ...
        dash = '----------------------------------------'
        msg_part = '\n'.join(card_descs)
        await session.send(f'别戳了别戳了o(╥﹏╥)o{card}{at_user}这些卡送给你了, 再戳屁屁都要肿了...\n{dash}\n获得了:\n{msg_part}')
